...
describe('User Model', () => {
    ...
    afterEach(async () => {
        await User_1.default.destroy({ where: {}, force: true });
    });
    describe('User Creation', () => {
        ...
        it('should not allow duplicate username', async () => {
            ...
            await expect(User_1.default.create(duplicateUserData)).rejects.toThrow();
        });
    ...
    describe('User Validation', () => {
        it('should validate username length', async () => {
            ...
            await expect(User_1.default.create(userData)).rejects.toThrow();
        });
    ...
    describe('Static Methods', () => {
        ...
        describe('findByEmail', () => {
            it('should find user by email', async () => {
                const user = await User_1.default.findByEmail('test@example.com');
                expect(user).toBeTruthy();
                expect(user?.email).toBe('test@example.com');
            });
            it('should return null for non-existent email', async () => {
                const user = await User_1.default.findByEmail('nonexistent@example.com');
                expect(user).toBeNull();
            });
        });
        describe('findByUsername', () => {
            it('should find user by username', async () => {
                const user = await User_1.default.findByUsername('testuser');
                expect(user).toBeTruthy();
                expect(user?.username).toBe('testuser');
            });
            it('should return null for non-existent username', async () => {
                const user = await User_1.default.findByUsername('nonexistent');
                expect(user).toBeNull();
            });
        });
        describe('hashPassword', () => {
            it('should hash password securely', async () => {
                ...
                const hash = await User_1.default.hashPassword(password);
