...
import HotnessService, { HotnessConfig } from '../services/hotnessService';

export class HotnessController {
  /**
   * 计算单个帖子的热度分数
   */
  static async calculatePostHotness(req: Request, res: Response): Promise<void> {
    try {
      ...
      // 获取可选的配置参数
      const config: Partial<HotnessConfig> = {};
      
      if (req.body.likeWeight !== undefined) config.likeWeight = parseFloat(req.body.likeWeight);
      if (req.body.commentWeight !== undefined) config.commentWeight = parseFloat(req.body.commentWeight);
      if (req.body.viewWeight !== undefined) config.viewWeight = parseFloat(req.body.viewWeight);
      if (req.body.gravity !== undefined) config.gravity = parseFloat(req.body.gravity);
      if (req.body.baseHours !== undefined) config.baseHours = parseFloat(req.body.baseHours);
      ...
      const result = await HotnessService.calculatePostHotness(postId, config);
   ...
   * 批量更新帖子热度分数
   */
  static async batchUpdateHotness(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await HotnessService.batchUpdateHotness(validPostIds, config);
   ...
   * 更新所有活跃帖子的热度分数
   */
  static async updateAllPostsHotness(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await HotnessService.updateAllActivePostsHotness(config);
   ...
   * 获取热门帖子列表
   */
  static async getHotPosts(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await HotnessService.getHotPosts(limit, minScore);
   ...
   * 基于时间范围获取热门帖子
   */
  static async getHotPostsByTimeRange(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await HotnessService.getHotPostsByTimeRange(hours, limit);
   ...
   * 获取热度算法配置
   */
  static async getHotnessConfig(req: Request, res: Response): Promise<void> {
    try {
      const config = HotnessService.getDefaultConfig();
   ...
   * 计算热度趋势
   */
  static async calculateHotnessTrend(req: Request, res: Response): Promise<void> {
    try {
      const postId = parseInt(req.params.id);
      ...
      const result = await HotnessService.calculateHotnessTrend(postId, config);
