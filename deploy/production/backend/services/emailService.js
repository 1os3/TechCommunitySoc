    ...
    static async initialize() {
        try {
            ...
            if (process.env.SMTP_HOST && process.env.SMTP_USER && process.env.SMTP_PASS) {
                ...
                logger_1.default.info(`Email service initialized with test account: ${testAccount.user}`);
            }
            // Verify connection
            if (this.transporter) {
        ...
        try {
            if (!this.transporter) {
                if (process.env.NODE_ENV === 'test') {
                    ...
                    logger_1.default.info(`[TEST] Email would be sent to ${options.to}: ${options.subject}`);
                    return true;
                }
            ...
            if (process.env.NODE_ENV === 'development') {
                ...
                if (previewUrl) {
                    logger_1.default.info(`Preview URL: ${previewUrl}`);
                }
            }
            return true;
        }
