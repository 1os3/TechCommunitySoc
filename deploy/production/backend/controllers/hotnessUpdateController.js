...
exports.HotnessUpdateController = void 0;
const hotnessUpdateService_1 = __importDefault(require("../services/hotnessUpdateService"));
const logger_1 = __importDefault(require("../utils/logger"));
class HotnessUpdateController {
    /**
     * 手动触发帖子热度更新
     */
    static async triggerPostUpdate(req, res) {
        try {
            if (!req.user) {
                res.status(401).json({
                    error: {
            ...
            const updateService = hotnessUpdateService_1.default.getInstance();
            ...
            logger_1.default.error('Trigger post update controller error:', error);
            res.status(500).json({
                error: {
     ...
     * 处理所有待更新的帖子
     */
    static async processAllPending(req, res) {
        try {
            if (!req.user) {
                res.status(401).json({
                    error: {
            ...
            logger_1.default.error('Process all pending controller error:', error);
            res.status(500).json({
                error: {
                    code: 'INTERNAL_SERVER_ERROR',
    ...
    static async getQueueStatus(req, res) {
        try {
            ...
            logger_1.default.error('Get queue status controller error:', error);
            res.status(500).json({
                error: {
                    code: 'INTERNAL_SERVER_ERROR',
    ...
    static async updateConfig(req, res) {
        try {
            ...
            logger_1.default.error('Update config controller error:', error);
            res.status(500).json({
                error: {
                    code: 'INTERNAL_SERVER_ERROR',
     ...
     * 清理过期触发器
     */
    static async cleanupExpiredTriggers(req, res) {
        try {
            if (!req.user) {
                res.status(401).json({
                    error: {
            ...
            logger_1.default.error('Cleanup expired triggers controller error:', error);
            res.status(500).json({
                error: {
                    code: 'INTERNAL_SERVER_ERROR',
     ...
     * 重置更新服务
     */
    static async resetService(req, res) {
        try {
            if (!req.user) {
                res.status(401).json({
                    error: {
            ...
            logger_1.default.error('Reset service controller error:', error);
            res.status(500).json({
                error: {
                    code: 'INTERNAL_SERVER_ERROR',
...
exports.HotnessUpdateController = HotnessUpdateController;
exports.default = HotnessUpdateController;
//# sourceMappingURL=hotnessUpdateController.js.map